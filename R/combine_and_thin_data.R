...
t = data2$time
X = as.matrix(lat_long_to_xy(data2$latitude, data2$longitude))
...
X = rbind(X, as.matrix(lat_long_to_xy(data3$LATITUDE, data3$LONGITUDE)) )
...
N <- dim(df)[1]
# time_dist = dist(df[,3])
# space_dist = dist(df[,1:2])

for (i in 1:(N-1)) {
  j <- i+1
  dt <- as.numeric(dist( rbind(df[i,3],df[j,3]) ))
  while (dt<=0.016 & j<=N) {
    ds <- as.numeric(dist( rbind(df[i,1:2],df[j,1:2]) ))
    if (ds < 0.1) df[j,] <- 0
    j <- j+1
    dt <- as.numeric(dist( rbind(df[i,3],df[j,3]) ))
  }
...
# #library(igraph)
# #ig = graph.adjacency(as.matrix(space_dist) < .1 & as.matrix(time_dist) <= .016  & lower.tri(space_dist),mode="lower")
# adj1 <- as.matrix(space_dist) < .1 
# adj2 <- as.matrix(time_dist) <= .016
# adj3 <- lower.tri(space_dist)
# n.orig = nrow(df)
...
# df = df[keep,]
  ...
  df_no_holidays_thinned <- df
  save(df_no_holidays_thinned,file = "data/no_holidays_thinned.Rdata")
